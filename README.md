...
| 当前 v2 强化候选 | Python 97/97、Node 22/22；最新双域 dry-run 制品仍为 304/304 | 新身份边界、并发事务、封口轨迹、原始 GUI 报告、能力胶囊和独立验证器通过本地回归 |
...
只有当模型选择的领域与规格编译器一致、版本化能力完整覆盖需求且工具参数被原样留存，内核路由才有资格通过。未知或新增能力只把对应需求节点交给受限代码 Agent；模型异常时可为了可用性生成既有内核，但不能通过发布门。
...
- 稳定领域合格路径限 1 次模型请求和 1 次 HTTP 尝试；重试可以帮助可用性，但当前发布门会失败。
