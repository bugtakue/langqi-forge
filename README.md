...
| 当前 v2 强化源码 | Python 126/126、Node 22/22；双域 dry-run 制品为 304/304 | 新身份边界、并发事务、锁定 Prompt/测试清单/Playwright 运行时、环境隔离、轨迹防泄露、独立验证器、第三域盲测通道和最小确定性比赛包通过本地回归 |
...
只有当模型选择的领域与规格编译器一致、版本化能力完整覆盖需求且原始 tool 参数被完整留存并可重演，内核路由才有资格通过。未知或新增能力只把对应需求节点交给受限代码 Agent；模型异常时即使可回滚基座已经物化，也会关闭预览、返回非零并拒绝发布。
...
未知域由受限代码 Agent 处理：工具只允许读写生成项目，写入受 SHA 前置条件、文件数与累计字节预算约束。历史模型/工具消息超过阈值时，Harness 会把它们封存在完整生产轨迹中，同时向模型提供确定性状态检查点。第一次实现校验通过后立即进入逐条验收审计；审计若补写任何文件，必须重新通过当前 revision 的校验才能完成。
