...
| Python 全仓测试 | 94 / 94 | — | 含证据篡改、路径逃逸、胶囊、Planner 时间预算、评委报告、竞品 runner 和实时违规终止合同 |
...
- 2026-09-01 的首个隔离盲测不构成成对成绩：Codex CLI `0.151.0` + `gpt-5.4/high` 在 1200.063 秒上限后 timeout，无违规工具事件但未产生可评分制品；Claude Code `2.1.161` 在 5.792 秒返回 OAuth token revoked 的 401，属认证失败而非参赛成绩。在 Claude 重新认证并完成同题运行前，不宣称任一通用 Agent 获胜。
