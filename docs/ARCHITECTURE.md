...
- 压缩后的原子需求摘要；
- 本地提出但不具权威性的候选领域标签；
- 该候选领域可用的版本化能力、完整行为边界与明确排除项。

模型回复中的原始 tool arguments 完整保留在 `model_response`，并单独哈希绑定到实际调用；实际调用使用同一个公开、确定性的规范化函数限制列表和文本长度。资格门会从原始回复重新规范化并与报告及已应用参数逐字段比对，所以兼容模型的冗长措辞不等于丢失原始证据，也不能借“清洗”偷换决策。
...
候选领域由确定性编译器提出，但不把本地覆盖结论交给 Planner。模型只收到该候选域的闭世界合同和候选域能力标签 schema，可以批准、否决或退回 generic；另一个领域的合同和标签不再重复占用 Token。等待这一次网络调用时，Harness 会并行物化可回滚的确定性基座并执行构建/健康预检。预检不会自行获得发布资格：没有后续代码改动时才晋级为最终本地检查；Planner 否决后则由受限 Agent 接管并重新全检。
