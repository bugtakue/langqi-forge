...
公开稳定域的 Planner 请求固定为 **1 次 HTTP attempt + 60 秒上限**（可用有界环境变量缩短）。资格门本来就拒绝重试后的“成功”，因此 Planner 失败时立即进入显式安全降级，不再用三次长等待消耗赛时。未知能力的 Coding Agent 仍有独立、可观测的请求与迭代预算。
...
| Planner 超时或网关失败 | 单次、有界等待后显式降级 | 失败；不在本次资格运行中重试 |
